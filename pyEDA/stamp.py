...
def stamp_c_mna(mna, element, ac=False):
    if ac:
...
def stamp_l_mna(mna, element, ac=False):
    mna = stamp_vsrc_mna(mna, element)
    index = mna.shape[0] - 1
    if ac:
...
def stamp_isrc_rhs(rhs, element):
    rhs[element.rhs1, 0] = rhs[element.rhs1, 0] + element.dc_value
    rhs[element.rhs2, 0] = rhs[element.rhs2, 0] - element.dc_value
    return rhs
...
def stamp_vccs_mna(mna, element):  # g
    ...
    mna[element.nc1, element.n2] -= element.value
    mna[element.nc2, element.n1] -= element.value
    return mna
